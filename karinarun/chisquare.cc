...
double  CalcChiSquared_pion(double A){
	FILE *fptr=fopen("results/cf_outsidelong.dat","r");
	FILE *fptr1=fopen("cf_exp.dat","r");
	double q,q1,cfout,cfout1,cfside1,cflong1,numout,cfside,numside,cflong,numlong,errorout, errorside, errorlong,chiout=0.0,chiside=0.0,chilong=0.0, chi2=0.0;
	int ipt;
	for(ipt=1;ipt<30;ipt++){
		fscanf(fptr,"%lf %lf %lf %lf %lf %lf %lf",&q,&cfout,&numout,&cfside,&numside,&cflong,&numlong);
		fscanf(fptr1,"%lf %lf %lf %lf",&q1,&cfout1,&cfside1,&cflong1);
		errorout=1/sqrt(numout);
		errorside=1/sqrt(numside);
		errorlong=1/sqrt(numlong);
